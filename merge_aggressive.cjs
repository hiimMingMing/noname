...
const aggressivePath = 'aggressive_translated.json';

const menu = JSON.parse(fs.readFileSync(menuPath, 'utf-8'));
const aggressive = JSON.parse(fs.readFileSync(aggressivePath, 'utf-8'));

console.log(`📦 Loading translations...`);
console.log(`   Menu: ${Object.keys(menu).length} keys`);
console.log(`   Aggressive batch: ${Object.keys(aggressive).length} keys`);

// Merge
let added = 0;
let skipped = 0;

for (const [cn, vi] of Object.entries(aggressive)) {
    if (!menu[cn]) {
        menu[cn] = vi;
        added++;
    } else {
