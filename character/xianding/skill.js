...
const skills = {
	...
	dcsbmingfang: {
		...
		async cost(event, trigger, player) {
			if (trigger.name == "phase") {
				event.result = await player
					.chooseTarget(get.prompt(event.skill), "令一名其他角色展示一张手牌，此牌在其手牌中时其受到伤害+1", (card, player, target) => {
			...
			event.result = await player
				.chooseTarget(get.prompt(event.skill), "与一名其他角色拼点，若你赢则对其造成1点伤害且本回合结束阶段可再次拼点，若你没赢则你与其横置", (card, player, target) => {
					return player != target && player.canCompare(target);
				})
				.set("ai", target => {
					const player = get.player();
					return get.damageEffect(target, player, player) / (target.countCards("h") + 1);
				})
				.forResult();
		},
		async content(event, trigger, player) {
			...
			if (trigger.name == "phase") {
				...
				const result = await target
					.chooseCard("展示一张手牌，此牌在你手牌中时你受到伤害+1", true, "h")
					.set("ai", card => {
						const player = get.player(),
							num = get.number(card);
				...
				if (result?.bool && result.cards?.length) {
					await target.showCards(result.cards, `${get.translation(player)}发动了【明访】`);
					const skill = "dcsbmingfang_damage";
					target.addSkill(skill);
					target.addGaintag(result.cards, skill);
				}
				return;
			}
				...
				player
					...
					.step(async (event, trigger, player) => {
						if (game.hasPlayer(current => player.canCompare(current))) {
							const result2 = await player
								.chooseTarget("明访：是否与一名角色拼点？", (card, player, target) => {
		...
		subSkill: {
			damage: {
				...
				filter(event, player) {
					return player.countCards("h", card => card.hasGaintag("dcsbmingfang_damage")) > 0;
				},
				forced: true,
				locked: false,
				async content(event, trigger, player) {
					trigger.num += player.countCards("h", card => card.hasGaintag(event.name));
				},
			},
		},
	},
	dcsbjibian: {
		...
		async cost(event, trigger, player) {
			const list = Array.from(Array(3)).map((v, i) => `${get.cnNumber(i + 1)}张`);
			const result = await player
			...
			player
				...
				.step(async (event, trigger, player) => {
					...
					if (trigger.target == player) {
						game.log(player, "的拼点牌点数-", num);
						trigger.num2 = Math.max(1, trigger.num2 - num);
					}
				});
		},
	},
	...
	dcsblingshu: {
		...
		chooseButton: {
			dialog(event, player) {
				...
				return ui.create.dialog("领戍", [vcards, "vcard"]);
			},
			check(button) {
			...
			prompt(links, player) {
				return "将一张非基本牌当" + (get.translation(links[0][3]) || "") + get.translation(links[0][2]) + "使用或打出";
			},
		},
		hiddenCard(player, name) {
			...
			return get.type(name) == "basic" && player.countCards("hes", card => _status.connectMode || get.type2(card) != "basic") > 0;
		},
		ai: {
			respondSha: true,
			respondShan: true,
			skillTagFilter(player) {
				...
				return player.countCards("hes", card => _status.connectMode || get.type2(card) != "basic") > 0;
			},
			order: 2,
			result: {
				player(player) {
	...
	dcsbwankang: {
		...
		async content(event, trigger, player) {
			...
			await player.draw();
			player.addTempSkill("dcsbwankang_sha");
		},
		marktext: "抗",
		intro: {
			name: "抗",
		...
		subSkill: {
			sha: {
				...
				async content(event, trigger, player) {
					while (true) {
						const targets = player.getHistory("damage").reduce((arr, evt) => {
							if (evt.source?.isIn()) {
								arr.add(evt.source);
							}
							return arr;
						...
						const next = player
							...
							}, "顽抗：是否对本回合对你造成过伤害的角色使用一张【杀】？")
							.set("targetRequired", true)
							.set("complexSelect", true)
							.set("complexTarget", true)
							.set("filterTarget", function (card, player, target) {
								if (!get.event("targetx").includes(target)) {
									return false;
								}
								return lib.filter.filterTarget.apply(this, arguments);
							})
							.set("oncard", (card, player) => {
								player.removeMark("dcsbwankang", 1);
							})
							.set("targetx", targets);
						...
						if (
							game.hasGlobalHistory("everything", evt => {
								...
								return evt.reason?.getParent(next.name) == next;
							})
						) {
							player.clearMark("dcsbwankang");
						}
						if (!result?.bool) {
							break;
						}
					}
					if (player.hasMark("dcsbwankang")) {
						await player.die();
					}
				},
			},
	...
	dcsbyinmou: {
		...
		filter(event, player) {
			let cards = [];
			event.player.checkHistory("lose", evt => {
				if (evt.hs?.length) {
					cards.addArray(evt.hs);
				}
			});
			return cards.length > event.player.countCards("h");
		},
		logTarget: "player",
		check(event, player) {
			let att = get.attitude(player, event.player);
			if (player.getStorage("dcsbyinmou", false)) {
				att *= -1;
			}
			return att > 0;
		},
		async content(event, trigger, player) {
			...
			const result = await player
				.chooseButton([`寅谋：选择一张令${get.translation(target)}获得`, cards], true)
				.set("bool", bool)
				.set("target", target)
				.set("ai", button => {
					const { player, target, bool } = get.event();
					let val = get.value(button.link, target) * get.attitude(player, target);
					if (bool) {
						val *= -1;
					}
					return val;
			...
			const next = target.gain(result.links, "draw");
			next.gaintag.add(`${event.name}${bool ? "_shadow" : ""}`);
			await next;
			game.log(player, "将牌堆顶的一张牌交给了", target);
		},
		group: ["dcsbyinmou_change"],
		subSkill: {
			effect: {
				trigger: {
					player: "loseAfter",
				...
				filter(event, player, name, card) {
					...
					if (Array.isArray(gaintag)) {
						if (gaintag.includes("dcsbyinmou")) {
							return player.countCards("h") < player.maxHp;
						}
						if (gaintag.includes("dcsbyinmou_shadow")) {
							return player.countDiscardableCards(player, "h") > 0 && player.getHp() > 0;
						}
					}
					return false;
				},
			...
			change: {
				...
				prompt2(event, player) {
					return "切换【寅谋】为状态" + (player.storage.dcsbyinmou ? "阳" : "阴");
				},
				check: () => Math.random() > 0.5,
				async content(event, trigger, player) {
					player.changeZhuanhuanji("dcsbyinmou");
				},
			},
		},
	},
	dcsbhongce: {
		...
		onChooseToUse(event) {
			...
			const evts = event.player.getAllHistory("custom", evt => evt.skill == "dcsbhongce");
			event.set("hongceEvts", evts);
		},
		filter(event, player) {
			if (event.name == "phase") {
				return game.roundNumber == 1;
			}
			return !player.hasSkill("dcsbhongce_used") && event.hongceEvts?.length;
		},
		filterTarget(card, player, target) {
			const { hongceEvts: evts } = get.event();
			const link = evts[0].link;
			return link == "draw" || target.countCards("h") > 0;
		},
		async cost(event, trigger, player) {
			const result = await player
				.chooseTarget("宏策：令一名角色选择一项执行", get.skillInfoTranslation(event.skill), true)
				.set("ai", target => {
					const player = get.player();
		...
		async content(event, trigger, player) {
			...
			if (trigger?.name == "phase") {
				...
				if (result?.bool && result.links?.length) {
					link = result.links[0];
					player.getHistory("custom").push({ skill: event.name, link: link });
				}
			} else {
				player.addTempSkill("dcsbhongce_used", { global: "phaseAnyAfter" });
				const evts = player.getAllHistory("custom", evt => evt.skill == event.name);
				if (evts.length) {
					link = evts[0].link;
				}
			}
			...
			switch (link) {
				...
				case "recast": {
					...
					const list = get.inpileVCardList(info => {
						...
						const check = () => {
							const select = infox.selectTarget;
							if (select !== undefined) {
				...
				default: {
					target.addSkill("dcsbhongce_limit");
					const next = target.draw(3);
					next.gaintag.add("dcsbhongce");
					await next;
					break;
				}
			}
		...
		subSkill: {
			...
			limit: {
				charlotte: true,
				onremove(player, skill) {
					player.removeGaintag("dcsbhongce");
				},
				mod: {
					ignoredHandcard(card, player) {
						if (card.hasGaintag("dcsbhongce")) {
							return true;
						}
					},
					cardDiscardable(card, player, name) {
						if (name == "phaseDiscard" && card.hasGaintag("dcsbhongce")) {
							return false;
						}
					},
				},
		...
		subSkill: {
			effect: {
				...
				filter(event, player) {
					...
					const list = player.getStorage("dcsbshibian_effect");
					if (!list.length) {
						return false;
					}
					return game.hasPlayer(current => {
				...
				async cost(event, trigger, player) {
					...
					event.result = await player
						.chooseTarget(get.prompt(event.skill), `令一名在本轮〖识变〗目标攻击范围内的角色成为${get.translation(trigger.card)}的额外目标`, (card, player, target) => {
				...
				mod: {
					inRange(from, to) {
						const list = from.getStorage("dcsbshibian_effect");
						if (!list.length) {
							return;
						}
						if (
	...
	dcsbbibu: {
		...
		filter(event, player) {
			...
			if (event.targets?.length != 1 || !lib.filter.targetEnabled(event.card, event.player, player)) {
				return false;
			}
		...
		async cost(event, trigger, player) {
			if (trigger.name == "useCard") {
				event.result = await player
					.chooseToDiscard(get.prompt(event.skill), `弃置一张牌并视为使用${get.translation(trigger.card)}`, "he")
					.set("ai", card => {
						const { eff, player } = get.event();
						return eff - get.value(card);
					})
					.set("chooseonly", true)
					.set("eff", player.getUseValue(new lib.element.VCard({ name: trigger.card.name, nature: trigger.card.nature, isCard: true })))
					.forResult();
				return;
			}
			event.result = await player
				.chooseBool(get.prompt(event.skill, trigger.target), `摸一张牌并将${get.translation(trigger.card)}的目标转移给你`)
				.set(
					"choice",
					(() => {
						let getE = current => get.effect(current, trigger.card, trigger.player, player);
						if (getE(trigger.target) > 0) {
							return false;
						}
						return getE(trigger.target) <= getE(player) + get.effect(player, { name: "draw" }, player, player);
					})()
				)
				.forResult();
	...
	dcsbxixing: {
		...
		async content(event, trigger, player) {
			...
			const bool = target.countGainableCards(player, "h");
			if (bool) {
	...
	dcsbjielu: {
		...
		async content(event, trigger, player) {
			...
			if (target.countDiscardableCards(player, "he")) {
				const { links } = await player.discardPlayerCard(target, true, "he").forResult();
				if (links?.length) {
		...
		onChooseToUse(event) {
			...
			const used = game
				.getGlobalHistory("useCard", evt => {
					return evt.card?.storage?.dcfuhui == player && (evt.cards ?? []).length == 2;
				})
	...
	dcmohua: {
		...
		check(event, player) {
			...
			const card = { name, storage: { dcmohua: history[0] } };
		...
		async content(event, trigger, player) {
			...
			for (const evt of history) {
				const { name } = evt.card;
				const card = (event.card = { name, storage: { dcmohua: evt } });
			...
			player.removeSkill("dcmohua_effect");
		},
		onChooseToUse(event) {
			const evt = event.getParent();
			if (evt.name === "dcmohua" && evt.player === event.player) {
				const evt2 = evt.card.storage.dcmohua;
		...
		subSkill: {
			effect: {
				...
				direct: true,
				filter(event, player) {
					const target = player.getStorage("dcmohua_effect");
				...
				mod: {
					selectTarget(card, player, range) {
						const source = player.getStorage("dcmohua_effect");
						if (!source.isIn() || get.itemtype(source) != "player") {
							return;
						}
						const evt = get.event().getParent("chooseToUse", true, true);
						if (!evt?.dcmohua) {
							return;
						}
			...
			draw: {
				...
				async content(event, trigger, player) {
					...
					const targets = evt.card.storage.dcmohua.targets.filter(target => trigger.targets.includes(target));
		...
		subSkill: {
			...
			directHit: {
				...
				async content(event, trigger, player) {
					...
					const { bool } = await target.chooseToDiscard("he", func, prompt).set("ai", card => {
						const player = get.player(),
							trigger = get.event().getTrigger();
	...
	dcshouzhi: {
		...
		async cost(event, trigger, player) {
			...
			if (delt < 0) {
				const bool = forced ? true : (await player.chooseBool(get.prompt(event.skill), "你可以摸两张牌。").forResult()).bool;
				event.result = { bool };
			} else {
	...
	diaodu: {
		...
		async content(event, trigger, player) {
			...
			if (bool) {
				...
				const { bool, cards } = await player.gainPlayerCard(aim, "e", true).forResult();
				if (bool && game.hasPlayer(target => target != aim)) {
					...
					if (bool && get.owner(card) == player) {
						...
						if (get.owner(card) == target) {
							const { bool } = await target.chooseUseTarget(card).forResult();
							if (bool) {
								await player.draw();
							} else {
								await target.draw();
	...
	zhengbi: {
		...
		direct: true,
		async content(event, trigger, player) {
			const { bool, targets }= await player.chooseTarget(get.prompt2("zhengbi"), lib.filter.notMe).set("ai", target => {
				const player = get.event("player");
			...
			if (bool) {
				...
				if (index == 0) {
					...
					const { bool } = await player.chooseToGive(target, { type: "basic" }, true).set("prompt", "征辟：交给" + str + "一张基本牌").forResult();
					if (bool) {
						let choices = [];
