...
class Create {
  ...
  editor(container, saveInput) {
    ...
    const createMenu = function(pos, self, List, click) {
      ...
      const editor2 = container.editor;
      if (!editor2) {
        return false;
      }
      self.style.background = "#08f";
      ...
      const theme2 = editor2.options.theme;
      lib.setScroll(ul);
      lib.setMousewheel(ul);
      ul.className = "CodeMirror-hints " + theme2;
      const getActive = () => {
        let i = 0;
    ...
    const theme = ui.create.div(".editbutton", "主题", editorpage, function() {
      ...
      if (!this.ul) {
        ...
        const click = function(e) {
          const theme2 = this.innerHTML;
          container.editor.setOption("theme", theme2);
          setTimeout(() => container.editor.refresh(), 0);
          game.saveConfig("codeMirror_theme", theme2);
          closeMenu.call(self);
        };
    ...
    const fontSize = ui.create.div(".editbutton", "字号", editorpage, function() {
      ...
      if (!this.ul) {
        ...
        const click = function(e) {
          ...
          Array.from(self.parentElement.children).map((v) => v.ul).filter(Boolean).forEach((v) => {
  ...
  async editor2(container, config = {}) {
    const {
      ...
      saveInput = (view2) => {
        ui.window.classList.remove("shortcutpaused");
        ui.window.classList.remove("systempaused");
        view2.dom.parentElement?.parentElement?.parentElement?.remove();
      }
    } = config;
    ...
    if (language === "javascript" || language === "typescript") {
      ...
      const { parser: htmlParser } = await __vitePreload(async () => {
        const { parser: htmlParser2 } = await import("../../../node_modules/.pnpm/@lezer_html@1.3.12/node_modules/@lezer/html/dist/index.js");
