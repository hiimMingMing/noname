...
class Player extends HTMLDivElement {
  ...
  buildEventListener(noclick) {
    ...
    if (noclick) {
      ...
      if (lib.config.touchscreen) {
        ...
        player2.addEventListener("touchmove", ui.click.playertouchmove);
      }
    }
  }
  ...
  addExtraEquip(skill, equip, replace = false, preserve) {
    ...
    player2.extraEquip.addArray(list);
    player2.$handleEquipChange();
    game.broadcast(
      (player3, list2) => {
        player3.extraEquip.addArray(list2);
        player3.$handleEquipChange();
      },
      player2,
      list
    );
  ...
  removeExtraEquip(skill, equip = "noequip") {
    ...
    if (equip != "noequip") {
      ...
      equips = player2.extraEquip.filter((info) => info[0] != skill || !equip.includes(info[1]));
    } else {
      equips = player2.extraEquip.filter((info) => info[0] != skill);
    }
    player2.extraEquip = equips;
    player2.$handleEquipChange();
    game.broadcast(
      (player3, equips2) => {
        player3.extraEquip = equips2;
        player3.$handleEquipChange();
      },
      player2,
      equips
  ...
  refreshSkill(skills) {
    ...
    for (const skill of skills) {
      ...
      if (info.round && player2.storage[skill + "_roundcount"]) {
        ...
        player2.unmarkSkill(skill + "_roundcount");
        resetSkills.add(skill);
      }
      if (player2.storage[`temp_ban_${skill}`]) {
...
hasSkillTag：`, arg);
  ...
  $handleEquipChange() {
    ...
    player2.extraEquip.forEach((info) => {
      if (player2.hiddenSkills.includes(info[0])) {
        return;
      }
