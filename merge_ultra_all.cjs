...
const ultraPath = 'ultra_all_translated.json';

const menu = JSON.parse(fs.readFileSync(menuPath, 'utf-8'));
const ultra = JSON.parse(fs.readFileSync(ultraPath, 'utf-8'));

console.log(`📦 Loading translations...`);
console.log(`   Menu: ${Object.keys(menu).length} keys`);
console.log(`   Ultra batch: ${Object.keys(ultra).length} keys`);

// Merge
let added = 0;
let skipped = 0;

for (const [cn, vi] of Object.entries(ultra)) {
    if (!menu[cn]) {
        menu[cn] = vi;
        added++;
    } else {
