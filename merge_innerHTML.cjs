...
const innerHTMLPath = 'innerHTML_translated.json';

const menu = JSON.parse(fs.readFileSync(menuPath, 'utf-8'));
const innerHTML = JSON.parse(fs.readFileSync(innerHTMLPath, 'utf-8'));

console.log(`📦 Loading translations...`);
console.log(`   Menu: ${Object.keys(menu).length} keys`);
console.log(`   innerHTML batch: ${Object.keys(innerHTML).length} keys`);

// Merge
let added = 0;
let skipped = 0;

for (const [cn, vi] of Object.entries(innerHTML)) {
    if (!menu[cn]) {
        menu[cn] = vi;
        added++;
    } else {
