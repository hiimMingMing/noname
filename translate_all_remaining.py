...
# MASSIVE character dictionary - Three Kingdoms + expansions
characters = {
    # Previous characters
    ...
    # Additional characters for batch 3
...
# MASSIVE skill dictionary - Vietnamese martial arts / historical terms
skills = {
    ...
    # New skills for batch 3
    "习事": "Tập Sự", "乡害": "Hương Hại", "乱击": "Loạn Kích",
    "乱年": "Loạn Niên", "乱战": "Loạn Chiến", "乱掠": "Loạn Lược",
    "乱朝": "Loạn Triều", "乱武": "Loạn Vũ", "乱群": "Loạn Quần",
    "乱魁": "Loạn Khôi", "乾纲": "Càn Cương", "争义": "Tranh Nghĩa",
    "争功": "Tranh Công", "争嗣": "Tranh Tự", "争擎": "Tranh Kình",
    "争適": "Tranh Thích", "于毒": "Vu Độc", "互忌": "Hỗ Kỵ",
    "互雠": "Hỗ Thù", "五星连珠": "Ngũ Tinh Liên Châu", "五灵": "Ngũ Linh",
    "五禽戏": "Ngũ Cầm Hí", "五虎": "Ngũ Hổ", "亡鹊": "Vong Thước",
    "亢勇": "Kháng Dũng", "亢悔": "Kháng Hối", "亢锐": "Kháng Nhuế",
    "交辉": "Giao Huy", "交锋": "Giao Phong", "亥猪": "Hợi Trư",
    "亦算": "Dịch Toán", "享乐": "Hưởng Lạc", "人公": "Nhân Công",
    "人望": "Nhân Vọng", "亿金": "Ức Kim", "仁仕": "Nhân Sĩ",
    "仁彀": "Nhân Cấu", "仁德": "Nhân Đức", "仁心": "Nhân Tâm",
    "仁政": "Nhân Chính", "仁智": "Nhân Trí", "仁望": "Nhân Vọng",
    "仁王": "Nhân Vương", "仁王金刚盾": "Nhân Vương Kim Cương Thuẫn",
...
# Special character names (Japanese)
japanese_names = {
...
def translate(text):
    ...
    # Handle descriptions (return None for now)
    if len(text) > 40 or '①' in text or '②' in text or '。' in text:
        return None

    # Handle & separator (dual characters)
    if '&' in text:
        parts = text.split('&')
        translated = []
        for part in parts:
    ...
    prefixes = [
        ("OL界", "OL Giới"),
        ("九鼎", "Cửu Đỉnh"),
        ("SP|赛马|神", "SP|Đua Ngựa|Thần"),
        ("SP赛马神", "SP Đua Ngựa Thần"),
        ("TW起", "TW Khởi"),
        ("TW将", "TW Tướng"),
        ("TW谋", "TW Mưu"),
        ("TW魔", "TW Ma"),
    ...
    for cn_prefix, vi_prefix in prefixes:
        if text.startswith(cn_prefix):
            base = text[len(cn_prefix):]
            if base in characters:
                return f"{vi_prefix} {characters[base]}"
            if base in skills:
                return f"{vi_prefix} {skills[base]}"

    return None

# Read files
with open('remaining_names_batch3.txt', 'r', encoding='utf-8') as f:
...
# Save
with open('batch3_translated.json', 'w', encoding='utf-8') as f:
...
print(f"💾 Saved to batch3_translated.json")
