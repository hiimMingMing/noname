...
const absolutePath = 'absolute_final_translated.json';

const menu = JSON.parse(fs.readFileSync(menuPath, 'utf-8'));
const absolute = JSON.parse(fs.readFileSync(absolutePath, 'utf-8'));

console.log(`📦 Loading translations...`);
console.log(`   Menu: ${Object.keys(menu).length} keys`);
console.log(`   Absolute final: ${Object.keys(absolute).length} keys`);

// Merge
let added = 0;
let skipped = 0;

for (const [cn, vi] of Object.entries(absolute)) {
    if (!menu[cn]) {
        menu[cn] = vi;
        added++;
    } else {
